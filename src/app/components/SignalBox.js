...
const SignalBox = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const avgVolume20 = candles.slice(-20).reduce((sum, c) => sum + parseFloat(c.volume), 0) / 20;
  ...
  const buySignal = signals?.trendUp && 
  ...
  const sellSignal = signals?.trendDown && 
                    signals?.trendStrength && 
    ...
    <div className="bg-zinc-900 p-4 rounded-xl shadow-md text-white w-full">
        ...
        {signals && (
          <>
            <p className={signals.trendUp ? 'text-green-500' : 'text-red-500'}>
              {signals.trendUp ? '✅ Trend Up (EMA20 > EMA50 > EMA200)' : '❌ Trend Down'}
            </p>
            <p className={signals.trendStrength ? 'text-green-500' : 'text-red-500'}>
              {signals.trendStrength ? '✅ Strong Trend (ADX > 25)' : '❌ Weak Trend'}
            </p>
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {buySignal ? (
              ...
              <tr>
                <td className="px-4 py-2" colSpan="4">در حال حاضر سیگنال بای مناسبی برای ورود وجود ندارد.</td>
              </tr>
            )}
          </tbody>
        </table>
        ...
        {signals && (
          <>
            <p className={signals.trendDown ? 'text-green-500' : 'text-red-500'}>
              {signals.trendDown ? '✅ Trend Down (EMA20 < EMA50 < EMA200)' : '❌ Trend Up'}
            </p>
            <p className={signals.trendStrength ? 'text-green-500' : 'text-red-500'}>
              {signals.trendStrength ? '✅ Strong Trend (ADX > 25)' : '❌ Weak Trend'}
            </p>
        ...
        <table className="w-full table-auto">
          ...
          <tbody>
            {sellSignal ? (
              ...
              <tr>
                <td className="px-4 py-2" colSpan="4">در حال حاضر سیگنال سل مناسبی برای ورود وجود ندارد.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {realtimePrice && (
        <div className="mt-6">
          ...
          <p className="text-white">حجم معاملات: {volumeFilter ? 'بالاتر از میانگین' : 'پایین‌تر از میانگین'}</p>
        </div>
      )}
    </div>
  );
